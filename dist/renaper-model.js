(function(root){
 ...
 function parse(text){const s=String(text).replace(/\s+/g,' ').trim();const m=s.match(/^(?:el\s+)?ejemplar\s+vigente\b.{0,180}?(?:es\s*:?|:)\s*(?:el\s+)?(?:ejemplar\s*)?["'«]?([A-Z]{1,2})["'»]?(?:,\s*con fecha de emisi[oó]n el \d{2}\/\d{2}\/\d{4})?[.\s]*$/i);return m&&m[1]===m[1].toUpperCase()&&!['NO','EL','ES','LA'].includes(m[1])?m[1]:'';}
